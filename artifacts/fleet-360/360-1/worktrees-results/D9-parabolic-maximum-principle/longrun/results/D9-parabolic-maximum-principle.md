...
- **Gate-repair 1:** `2026-09-10T01:49:05+00:00` (UTC) — added the missing worktree-root
  Lake workspace so the longrun compile gate (`lake env lean <file>`, cwd = worktree root)
  resolves the prebuilt release oleans. **No D9 mathematical file changed.**
...
> **Gate-repair note (attempt 1).** The longrun compile gate runs
> `lake env lean <absolute path>` with **cwd = the worktree root** for every `.lean` file in
> the tree (see `dispatch360.compile_gate`). The D6 scaffold only carried
> `release/lakefile.toml`, so from the worktree root Lake found no workspace at all and the
> first gate failed on *every* file even though the four D9 modules compiled cleanly from
> `release/`. The repair added the missing worktree-root wrapper — `lakefile.toml`
> (library `Poincare`, `srcDir = "release"`, `globs = ["Poincare.+"]`, with the same
> `lean-toolchain` pin `leanprover/lean4:v4.34.0-rc2` and the same mathlib revision
> `7974e751bece493b6ff508039423ca9fa2452fa8`) plus the symlink `.lake -> release/.lake`,
> so the gate reuses the prebuilt oleans and mathlib without rebuilding or re-fetching
> anything. All **68** `.lean` files of the worktree now exit 0 under the exact gate command
> (`logs/gate_replicate.json`, `logs/gate_replicate.stdout`); the four D9 files are
> byte-identical to the hashes in §1, so nothing mathematical changed in the repair.
...
| **worktree compile gate** | `python3 logs/gate_replicate.py` — byte-for-byte replication of `dispatch360.compile_gate`: `lake env lean <file>` with cwd = worktree **root** for **all 68** `.lean` files | 0 | `logs/gate_replicate.json` (**68/68 exit 0**), `logs/gate_replicate.stdout` |
...
The worktree compile gate is the command that actually failed the first gate attempt (see
the gate-repair note above). After the root wrapper was added it passes for the complete
file set under the worktree — not only for the four D9 modules from `release/` — with the
same `ELAN_HOME`/`PATH`/`NODE_OPTIONS` environment and the same walk order as
`dispatch360.compile_gate`.
...
The compile gate the dispatcher runs is the worktree-root one; reproduce it (all 68 files)
with:

```bash
cd /data/home/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D9-parabolic-maximum-principle
export ELAN_HOME=/data/home/guoshaoyang/workdir/lean_poincare/elan
export PATH="$HOME/.local/node/bin:$ELAN_HOME/bin:$PATH"
export TMPDIR=/tmp
python3 logs/gate_replicate.py   # prints "<exit>\t<seconds>\t<file>" per file, then OK
```

## 9. Repair status

- **Gate-repair attempt 1: complete.** Root cause: the worktree had no worktree-root Lake
  workspace, so the gate's `lake env lean <file>` (cwd = worktree root) could not resolve
  mathlib or the release oleans. Fix: root `lakefile.toml` + `.lake -> release/.lake`; no
  D9 `.lean` file was touched (all four hashes in §1 still match).
- Fresh forbidden-token scan over the four D9 files: **4 scanned, 0 hard matches, 0 soft
  matches, exit 0** (`logs/D9_forbidden_scan_repair2.json`).
- Kernel audit unchanged: **49/49** declarations in `[propext, Classical.choice, Quot.sound]`,
  **0** `sorryAx` / custom axiom / `unsafe` / `native_decide` / `proof_wanted`.
- Gate replication: **68/68** `.lean` files exit 0 (`logs/gate_replicate.json`).
- **Independent re-verification (second pass, 2026-09-10T10:06:02+0800).** Re-ran the full
  worktree-root per-file gate (`python3 logs/gate_replicate.py`): **68/68** `.lean` files
  exit 0, aggregate `ok: true`, exit codes identical to the dispatcher `gate.json`
  (`logs/gate_replicate.json`, `logs/gate_replicate2.stdout`). Re-ran
  `input/d5-tools/scan_forbidden.py release/Poincare/D9/ParabolicMaximumPrinciple`:
  **4 scanned, 0 hard matches, 0 soft matches**, exit 0. No `.lean` file was modified in
  this pass (newest D9 source timestamp 2026-09-10 09:09).
