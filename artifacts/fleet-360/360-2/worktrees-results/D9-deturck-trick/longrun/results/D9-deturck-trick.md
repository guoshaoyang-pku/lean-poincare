...
- **Repair attempt 1:** `2026-09-09T16:01:44Z` — compile-gate environment fixed at the worktree root; all 63 `.lean` files now exit 0 under the gate command (see §6.1). The four authored modules are unchanged (sha256 unchanged; §1).
...
All authored files live under `release/Poincare/D9/DeTurck/`.
...
| `SymbolModel.lean` | 306 | `d66a116080ae6c14` | finite-dimensional symbol model on `MetricData`: `metricSharp`, `covectorNormSq`, `bilinTrace`, `ricciSymbol`, `deTurckFieldSymbol`, `lieSymbol`, `laplacianSymbol`; the principal-symbol identity and its flow form; symmetry preservation; ellipticity |
...
| `All.lean` | 19 | `91c66c64fb5bd4e9` | aggregate driver importing the three modules |
...
- forbidden-token scan (`input/d5-tools/scan_forbidden.py release/Poincare/D9/DeTurck`):
  **4 files scanned, 0 hard matches, 0 soft matches** (`logs/D9_deturck_forbidden_scan.json`)
...
| `lake env lean Poincare/D9/DeTurck/All.lean` | 0 |
| `lake build Poincare.D9.DeTurck.All` | 0 (3074 jobs) |
...
The harness compile gate (`dispatch360.py :: compile_gate`) does **not** compile from
`release/`: it walks every `.lean` file under the worktree and runs
...
and every one of the 63 `.lean` files failed immediately with
...
(`rsync --exclude='.lake/'`) and the central gate re-run. With these three files the gate
command succeeds for every `.lean` file in the worktree:

| command | files | exit 0 | exit ≠ 0 |
|---|---|---|---|
| `cd <worktree root> && lake env lean <f>` (all `.lean` files) | 63 | 63 | 0 |

Per-file exit codes and wall times: `logs/D9_deturck_gate_replica_root.log` (63/63 exit 0,
max 5.1 s per file). The four authored modules are byte-identical to the ones described above
(sha256 in the JSON card), and all commands of §6 still exit 0.
...
for f in ConnectionLayer SymbolModel StatementOnly All; do
  lake env lean "Poincare/D9/DeTurck/${f}.lean" || exit 1
done
lake build Poincare.D9.DeTurck.All
python3 ../input/d5-tools/scan_forbidden.py Poincare/D9/DeTurck
# harness gate, reproduced exactly (worktree root as cwd):
cd .. && for f in $(find . -path ./.lake -prune -o -path ./.git -prune -o \
...
modified; the only added files are the four modules above, the three root-level gate-support
files of §6.1, the result card, and the logs.
...
- `release/Poincare/D9/DeTurck/All.lean`
...
- `logs/D9_deturck_gate_replica_root.log` (all 63 worktree `.lean` files, exit 0)
