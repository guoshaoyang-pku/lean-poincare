/- GENERATED by baseline/tools/gen_axiom_audit.py — do not edit by hand. -/
...
namespace L1BaselineAudit

/-- Modules under audit: every authored release module Lake compiled. -/
...
run_cmd do
  ...
  let mut nSorry := 0
  let mut nNative := 0
  let mut nUnexpected := 0
  let mut nExpectedSeen := 0
  ...
  for (n, ci, m) in rows do
    ...
    -- direct intra-package dependency edges (type + value)
    ...
    IO.println s!"L1AXROW\t{n}\t{kind}\t{m}\t{String.intercalate "," (axs.map toString)}\t{String.intercalate "," (extra.map toString)}\t{n.isInternal}"
  ...
  let ok := nUnexpected == 0 && nCollectFail == 0 && nSorry == 0
    && nUnsafe == 0 && nNative == 0 && nExpectedSeen == expectedBad.size
  if ok then
    IO.println "L1AXVERDICT\tPASS — every audited declaration's axiom cone ⊆ {propext, Classical.choice, Quot.sound}; both negative controls detected"
  else
    IO.println "L1AXVERDICT\tFAIL"
    throwError "L1 baseline axiom audit FAILED (unexpected={nUnexpected}, collect_fail={nCollectFail}, sorry={nSorry}, unsafe={nUnsafe}, native={nNative}, expected_seen={nExpectedSeen}/{expectedBad.size})"
