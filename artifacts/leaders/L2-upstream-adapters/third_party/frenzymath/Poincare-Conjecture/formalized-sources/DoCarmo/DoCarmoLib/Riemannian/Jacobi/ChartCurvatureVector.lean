...
`DoCarmoLib.Riemannian.Jacobi.CovariantCurvatureAlong` with the actual Christoffel data of a
Riemannian metric `g` in the chart at `α`, from DoCarmoLib's chart API:
...
(`DoCarmoLib.Riemannian.Geodesic.LinearODE`) and actual
