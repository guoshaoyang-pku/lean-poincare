/- Petersen's own Riemannian infrastructure.
   Originally derived from the DoCarmo project's `DoCarmoLib/Riemannian/Exponential/MinimizingPiecewise.lean`; it is maintained
...
`DoCarmoLib.PetersenLib.Exponential.Minimizing` from a single differentiable
