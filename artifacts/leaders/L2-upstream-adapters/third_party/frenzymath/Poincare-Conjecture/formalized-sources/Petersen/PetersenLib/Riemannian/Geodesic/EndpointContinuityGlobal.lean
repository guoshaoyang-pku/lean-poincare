/- Petersen's own Riemannian infrastructure.
   Originally derived from the DoCarmo project's `DoCarmoLib/Riemannian/Geodesic/EndpointContinuityGlobal.lean`; it is maintained
   here independently and is engineering support, not a blueprint node. -/
import PetersenLib.Riemannian.Geodesic.EndpointContinuity
...
`PetersenLib.Exponential.completeSpace_of_forall_geodesic` directly.
-/
