...
OUT="/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/leaders/L4-geometric-critical-path/logs/round5/full-sweep.log"
: > "$OUT"
FILES=$(cd "$ROOT" && find . -name '*.lean' -not -path './.lake/*' | sed 's|^\./||' | sort)
TOTAL=$(echo "$FILES" | wc -l)
echo "=== ROUND 5 RELEASE-WIDE AUTHORED SWEEP $(date -u +%Y-%m-%dT%H:%M:%SZ) TOTAL=$TOTAL ===" >> "$OUT"
export TC ROOT OUT
compile_one() {
  f="$1"
