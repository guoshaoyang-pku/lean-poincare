...
/-- **Volume doubling from the Jacobi Bishop–Gromov ratio.**  Under the hypotheses of
`jacobi_bishopGromovVolumeRatio`, the cumulative radial volume satisfies the quantitative
doubling inequality `V (2r) ≤ 4 · V r` on `(0,T]` whenever `2r ≤ T`.  This is the shape of
the volume-doubling input consumed by compactness/covering arguments; here it is *derived*
from the comparison theorem (and the Euclidean model `V̄ t = t²/2`), not assumed.  The
constant `4 = V̄(2r)/V̄(r)` is the flat two-dimensional doubling constant. -/
theorem jacobi_radialVolume_doubling {k u du ddu : ℝ → ℝ} {T B t₀ : ℝ}
    (hT : 0 < T) (hBnn : 0 ≤ B) (ht₀ : 0 < t₀) (ht₀T : t₀ ≤ T) (hBt₀ : B * t₀ ≤ 1 / 2)
    (h : JacobiSolutionOn k u du ddu 0 T) (hdducont : ContinuousOn ddu (Icc 0 T))
    (hB : ∀ t ∈ Ioo 0 T, |ddu t| ≤ B) (hu0 : u 0 = 0) (hdu0 : du 0 = 1)
    (hpos : ∀ t ∈ Ioc 0 T, 0 < u t) (hk : ∀ t ∈ Ioo 0 T, 0 ≤ k t)
    {r : ℝ} (hr : r ∈ Ioc 0 T) (h2r : 2 * r ∈ Ioc 0 T) :
    radialVolume u (2 * r) ≤ 4 * radialVolume u r := by
  have hratio := jacobi_bishopGromovVolumeRatio hT hBnn ht₀ ht₀T hBt₀ h hdducont hB hu0
    hdu0 hpos hk hr h2r (by linarith [hr.1])
  have hVb2 : radialVolume (euclidModelA 1) (2 * r) = (2 * r) ^ 2 / 2 := by
    rw [euclidModel_volume (d := 1) (2 * r)]
    norm_num
  have hVb1 : radialVolume (euclidModelA 1) r = r ^ 2 / 2 := by
    rw [euclidModel_volume (d := 1) r]
    norm_num
  rw [hVb2, hVb1] at hratio
  have hrpos : 0 < r := hr.1
  have h2pos : 0 < (2 * r) ^ 2 / 2 := by positivity
  have hr2pos : 0 < r ^ 2 / 2 := by positivity
  have hmul := (div_le_div_iff₀ h2pos hr2pos).mp hratio
  nlinarith [hmul, sq_nonneg r, sq_nonneg (r - 1)]
...
/-- **Doubling witness.**  The sine model satisfies every hypothesis of
`jacobi_radialVolume_doubling`, giving the quantitative doubling inequality
`V (2r) ≤ 4 V r` for `r ∈ (0,1/4]` (so that `2r ≤ 1/2`). -/
theorem sin_radialVolume_doubling :
    ∀ ⦃r : ℝ⦄, r ∈ Ioc 0 (1 / 4) → radialVolume Real.sin (2 * r) ≤ 4 * radialVolume Real.sin r := by
  intro r hr
  have hmain := jacobi_radialVolume_doubling (T := 1 / 2) (B := 1) (t₀ := 1 / 2) (r := r)
    (k := fun _ : ℝ => 1) (u := Real.sin) (du := Real.cos) (ddu := fun t => -Real.sin t)
    (by norm_num) (by norm_num) (by norm_num) le_rfl (by norm_num)
    sin_jacobiSolution
    (by fun_prop)
    ...
    (fun t ht => Real.sin_pos_of_pos_of_lt_pi ht.1 (by linarith [ht.2, Real.pi_gt_three]))
    (fun t ht => by norm_num)
  exact hmain ⟨hr.1, by linarith [hr.2]⟩ ⟨by linarith [hr.1], by linarith [hr.2]⟩
