...
* `expMap x v = γ x v 1 = x + v` — the exponential map of the flat model.  Its injectivity
  (`expMap_injective`) is the elementary model analogue of the absence of conjugate points;
  conjugate points are a *manifold-level* notion and are **not** defined or proved here (see the
  not-claimed list below);
...
/-- The flat exponential map is injective.  This is an elementary translation fact
(`add_left_cancel`); see the semantic classification: conjugate points are not defined or
...
/-- **The model radial field has no second zero**: for `v ≠ 0`, the radial Jacobi field
`J t = t • v` vanishes only at `t = 0`.  This is a torsion-freeness statement in a real vector
space, not a conjugate-point theorem. -/
