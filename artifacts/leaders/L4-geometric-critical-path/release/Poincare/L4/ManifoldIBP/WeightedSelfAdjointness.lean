...
* `halfSpaceAtlas_dirichletEnergy_nonneg` — **model** (proved): the weighted Dirichlet
  ...
  integrand is integrable.  The integrability conjunct is D13's
  `halfSpaceAtlas_integrable_dirichlet` (it is where the glued measure is used); the sign
  conjunct is the measure-agnostic pointwise positive semidefiniteness, which D13 does not
  record.  D13 proves the Dirichlet *identity* and the integrability but not the *sign*.
