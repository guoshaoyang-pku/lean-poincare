/-
L5-topology-audit — independent fail-closed kernel detector, Pass B — audits the `Poincare.D7.Monotonicity.*` / `Poincare.D7.Kappa.*` side (and every collision-free module except the 25 modules whose transitive closure imports the `D7.Bochner.*`/`D7.ConjugateHeat.Basic` originals).

437 modules imported.  The union of pass A and pass B covers all 462 buildable
