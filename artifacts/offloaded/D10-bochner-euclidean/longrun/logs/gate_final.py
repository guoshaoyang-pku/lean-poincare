#!/usr/bin/env python3
"""Exact replica of dispatch360.compile_gate for local verification.

Walks the worktree (excluding .lake/.git/.dshpkg), runs `lake env lean <file>`
with cwd = worktree root, and writes per-file exit codes plus failure tails to
longrun/logs/D10-gate-final.json.  Never writes state/<task>/gate.json.
"""
import json, os, subprocess, time

WT = "/data/home/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D10-bochner-euclidean"
OUT = os.path.join(WT, "longrun", "logs", "D10-gate-final.json")
