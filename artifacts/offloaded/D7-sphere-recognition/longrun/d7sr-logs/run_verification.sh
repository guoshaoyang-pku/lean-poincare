#!/usr/bin/env bash
# Reproduce the D7-sphere-recognition verification transcript.
#
# Usage:  bash longrun/d7sr-logs/run_verification.sh
#
# Steps:
...
#   5. source-integrity check against the stable dependency scaffold
set -u
ROOT=/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D7-sphere-recognition
cd "$ROOT"
LOGS=longrun/d7sr-logs
mkdir -p "$LOGS"

FILES="Basic Assembly Homeomorphism All Probe Audit"
...
for f in $FILES; do
  lake env lean "release/Poincare/D7/Recognition/$f.lean" > "$LOGS/lean_$f.out" 2> "$LOGS/lean_$f.err"
  code=$?
  echo "$f $code" >> "$LOGS/exit_codes.txt"
done

# 3. forbidden-token scan
python3 input/d5-tools/scan_forbidden.py release/Poincare/D7/Recognition \
...
# 5. source integrity
python3 "$LOGS/source_integrity.py" > "$LOGS/source-integrity.json" 2> "$LOGS/source-integrity.err"
