#!/usr/bin/env bash
# Reproduce the D7-kappa-noncollapsing-conditional verification transcript.
#
# Usage:  bash longrun/d7knc-logs/run_verification.sh
#
# Steps:
...
set -u
ROOT=/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D7-kappa-noncollapsing-conditional
cd "$ROOT"
LOGS=longrun/d7knc-logs
mkdir -p "$LOGS"

FILES="Basic EntropyBridge Statements Nonvacuity All Probe Audit"
...
for f in $FILES; do
  lake env lean "release/Poincare/D7/Kappa/$f.lean" > "$LOGS/lean_$f.out" 2> "$LOGS/lean_$f.err"
  code=$?
  echo "$f $code" >> "$LOGS/exit_codes.txt"
done

# 3. forbidden-token scan
python3 input/d5-tools/scan_forbidden.py release/Poincare/D7/Kappa \
