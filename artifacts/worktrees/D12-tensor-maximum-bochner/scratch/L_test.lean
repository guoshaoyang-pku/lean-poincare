...
example {n : Type*} [Fintype n] {v : n → ℝ} {A B : n → n → ℝ} :
  ...
  simp only [dotProduct, Matrix.mulVec, add_mul, mul_add, Finset.sum_add_distrib, Finset.mul_sum]

example {n : Type*} [Fintype n] {a : ℝ} {v : n → ℝ} {A : n → n → ℝ} :
    v ⬝ᵥ (Matrix.mulVec (a • A) v) = a * (v ⬝ᵥ (Matrix.mulVec A v)) := by
  simp only [dotProduct, Matrix.mulVec, smul_eq_mul, mul_assoc, mul_left_comm, mul_comm,
