...
lemma weitz_coefficient (d : LeviCivitaData m b) (j p : ι) :
  ...
  rw [show (1 / 2 : ℝ) • (∑ a : ι, ∑ b : ι, (K d j a b * cG d a b p : A))
    ...
    have h2 : (∑ a : ι, ∑ b : ι, (K d j a b * cG d a b p : A))
        = 2 • (∑ a : ι, ∑ b : ι, (K d j a b * gamma d a b p : A)) := by
      calc
        (∑ a : ι, ∑ b : ι, (K d j a b * cG d a b p : A))
          ...
          _ = (∑ a : ι, ∑ b : ι, (K d j a b * gamma d a b p : A))
              + (∑ a : ι, ∑ b : ι, (K d j a b * gamma d a b p : A)) := by
                ...
                apply Finset.sum_congr rfl
                intro b _
                have hk : K d j b a = - K d j a b := by
                  have h := K_antisym (A := A) d j a b
                  rw [show K d j a b + K d j b a = K d j b a + K d j a b by abel] at h
                  exact eq_neg_of_add_eq_zero_left h
                rw [hk]
                rw [show - K d j a b * gamma d a b p = - (K d j a b * gamma d a b p) by ring]
                ring
          _ = 2 • (∑ a : ι, ∑ b : ι, (K d j a b * gamma d a b p : A)) := by
                rw [← two_smul]
                rw [Finset.sum_smul]
                congr 1
                apply Finset.sum_congr rfl
    ...
    calc
      (1 / 2 : ℝ) • (∑ a : ι, ∑ b : ι, (K d j a b * cG d a b p : A))
          = (1 / 2 : ℝ) • (2 • (∑ a : ι, ∑ b : ι, (K d j a b * gamma d a b p : A))) := by
              rw [h2]
