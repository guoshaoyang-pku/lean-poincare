#!/usr/bin/env python3
"""Round-6 producer-card freeze re-check.

Re-hash every producer card `.md`/`.json` recorded in `card_freeze_round4.json`
and confirm the bytes are still identical to the round-2/3/4 frozen state, so the
round-6 verdicts are not stale.
"""
import hashlib
import json
import os
...
for name, rec in freeze["files"].items():
    ...
    if not same:
        out["all_unchanged"] = False
with open(os.path.join(HERE, "card_freeze_round6.json"), "w") as f:
