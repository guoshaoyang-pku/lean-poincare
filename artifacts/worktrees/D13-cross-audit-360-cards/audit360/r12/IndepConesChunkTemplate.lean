-- D13-cross-audit-360-cards round 12 (invocation 9), adversarial lane A3: CHUNKED variant.
--
...
--   * progress lines `A3R12PROGRESS|<roots_done>|<roots_total>|<memo>` so a
...
run_cmd do
  ...
  unless axPos.contains `a3r12TestAxiom do
    throwError "A3R12 SELFTEST FAIL: traversal missed the instrument axiom"
  unless axNeg.isEmpty do
    throwError "A3R12 SELFTEST FAIL: traversal invented axioms {axNeg.toList}"
  logInfo m!"A3R12 SELFTEST PASS (positive control detected, negative control clean)"
  -- 2. every constant under the D12 root, CHUNK-th of NCHUNK modulo partition
  -- (cones are per-root, so partitioning the roots across processes is exact;
  -- only the shared memo is not reused across chunks)
  let mut roots : Array Name := #[]
  let mut idx : Nat := 0
  for (n, _ci) in env.constants.toList do
    if root.isPrefixOf n && !excluded.contains n then
      if idx % NCHUNK == KCHUNK then roots := roots.push n
      idx := idx + 1
  let mut st : A3R12.ConeState := {}
  for i in [:roots.size] do
    st := A3R12.coneStep env st roots[i]!
    if (i + 1) % 20 == 0 || i + 1 == roots.size then
      logInfo m!"A3R12PROGRESS|{i + 1}|{roots.size}|{st.memo.size}"
  let cones := st.memo
  ...
  for n in roots do
    ...
    logInfo m!"A3R12CONE|{n}|{String.intercalate "," (axs.toList.map Name.toString)}|0"
  logInfo m!"A3R12 TOTAL {roots.size} declarations, memo size {cones.size}"
  for (n, u) in bad do
    logError m!"A3R12 BAD {n} -> {u.map Name.toString}"
  if !bad.isEmpty then
    throwError "A3R12 FAIL: {bad.size} declarations with unapproved transitive axioms"
  logInfo m!"A3R12 PASS"
