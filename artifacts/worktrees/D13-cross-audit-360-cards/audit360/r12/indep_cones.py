#!/usr/bin/env python3
"""Round-12 independent transitive axiom-cone checker (adversarial lane A3).
...
`A3R12CONE` / `A3R12PROGRESS` / `A3R12 TOTAL` lines.

Usage: python3 audit360/r12/indep_cones.py [card ...]
Outputs: audit360/r12/indep_cones_<card>.log/.json, indep_cones.json
"""
import hashlib
import json
import os
...
PROG = re.compile(r"A3R12PROGRESS\|(\d+)\|(\d+)\|(\d+)")
ENV = dict(os.environ)
...
def main():
    ...
    for card in cards:
        ...
        with open(log, "w", encoding="utf-8") as fh:
            try:
                proc = subprocess.run(
                    ["lake", "env", "lean", "A3ExtraR12/IndepCones.lean"],
        ...
        if progs:
            last_prog = {"roots_done": int(progs[-1][0]), "roots_total": int(progs[-1][1]),
                         "memo": int(progs[-1][2])}
        selftest = "A3R12 SELFTEST PASS" in text
        passed = "A3R12 PASS" in text
        ...
        sys.stdout.flush()
    with open(os.path.join(HERE, os.environ.get("A3R12_OUT", "indep_cones.json")), "w",
