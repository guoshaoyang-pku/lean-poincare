...
-- D13-cross-audit-360-cards round 12 (invocation 9), adversarial lane A3: SCOPED variant (local D12 proof graph; imported constants are leaves, recorded only if they are axioms).
--
...
--   * progress lines `A3R12SPROGRESS|<roots_done>|<roots_total>|<memo>` so a
...
def coneStep (env : Environment) (root : Name) (st : ConeState) (r : Name) : ConeState := Id.run do
  ...
  while !stack.isEmpty do
    ...
    if finish then
      ...
      let (refs, rm) : Array Name × Std.HashMap Name (Array Name) :=
        if root.isPrefixOf n then
          match refMemo.get? n with
          | some rs => (rs, refMemo)
          | none =>
            let rs := directRefs env n false
            (rs, refMemo.insert n rs)
        else (#[], refMemo)
      refMemo := rm
      let mut s : Std.HashSet Name := {}
      for c in refs do
...
run_cmd do
  ...
  let st0 := A3R12.coneStep env `a3r12TestUsesAxiom {} `a3r12TestUsesAxiom
  let st1 := A3R12.coneStep env `a3r12TestNoAxiom st0 `a3r12TestNoAxiom
  ...
  unless axPos.contains `a3r12TestAxiom do
    throwError "A3R12S SELFTEST FAIL: traversal missed the instrument axiom"
  unless axNeg.isEmpty do
    throwError "A3R12S SELFTEST FAIL: traversal invented axioms {axNeg.toList}"
  logInfo m!"A3R12S SELFTEST PASS (positive control detected, negative control clean)"
  -- 2. every constant under the D12 root, one memoised pass with progress
  ...
  for i in [:roots.size] do
    st := A3R12.coneStep env root st roots[i]!
    if (i + 1) % 20 == 0 || i + 1 == roots.size then
      logInfo m!"A3R12SPROGRESS|{i + 1}|{roots.size}|{st.memo.size}"
  let cones := st.memo
  ...
  for n in roots do
    ...
    logInfo m!"A3R12SCONE|{n}|{String.intercalate "," (axs.toList.map Name.toString)}|0"
  logInfo m!"A3R12S TOTAL {roots.size} declarations, memo size {cones.size}"
  for (n, u) in bad do
    logError m!"A3R12S BAD {n} -> {u.map Name.toString}"
  if !bad.isEmpty then
    throwError "A3R12S FAIL: {bad.size} declarations with unapproved transitive axioms"
  logInfo m!"A3R12S PASS"
