#!/usr/bin/env python3
"""Round-11 (invocation 8) close-out: compare the round-11 sweep outputs with round 10
byte-for-byte (ignoring `###` header lines and lake's `[k/N]` job counters) and
emit audit360/round11_summary.json.
"""
import json
import os
import re
...
LOGS = os.path.join(HERE, "logs-round11")
PREV = os.path.join(HERE, "logs-round10")
KINDPREV = os.path.join(HERE, "logs-round10")
CARDS = [
...
def main():
    summary = {"round": 11, "invocation": 8,
               "generated_by": "audit360/round11_summary.py",
               "logs": "audit360/logs-round11", "compared_to": "audit360/logs-round10"}
    cards = {}
    for card in CARDS:
        entry = {}
        for suffix in ("build", "probe", "fullaudit", "kind"):
            ...
            entry[f"{suffix}_identical_to_round10"] = (
    ...
    summary["all_core_identical_to_round10"] = all(
        c["build_identical_to_round10"] and c["probe_identical_to_round10"]
        and c["fullaudit_identical_to_round10"] and c["kind_identical_to_round10"]
    ...
    with open(os.path.join(HERE, "round11_summary.json"), "w") as f:
