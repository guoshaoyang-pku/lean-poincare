#!/usr/bin/env python3
"""Round-6 (invocation 5) close-out: compare the round-6 sweep outputs with round 5
byte-for-byte (ignoring `###` header lines and lake's `[k/N]` job counters) and
emit audit360/round6_summary.json.
"""
import json
import os
import re
...
LOGS = os.path.join(HERE, "logs-round6")
PREV = os.path.join(HERE, "logs-round5")
KINDPREV = os.path.join(HERE, "logs-kind")  # round-5 kind screen was logged here
CARDS = [
...
def main():
    summary = {"round": 6, "invocation": 5,
               "generated_by": "audit360/round6_summary.py",
               "logs": "audit360/logs-round6", "compared_to": "audit360/logs-round5"}
    cards = {}
    for card in CARDS:
        entry = {}
        for suffix in ("build", "probe", "fullaudit", "kind"):
            ...
            entry[f"{suffix}_identical_to_round5"] = (
    ...
    summary["all_core_identical_to_round5"] = all(
        c["build_identical_to_round5"] and c["probe_identical_to_round5"]
        and c["fullaudit_identical_to_round5"] and c["kind_identical_to_round5"]
    ...
    with open(os.path.join(HERE, "round6_summary.json"), "w") as f:
