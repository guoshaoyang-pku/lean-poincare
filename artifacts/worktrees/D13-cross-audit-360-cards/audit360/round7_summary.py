#!/usr/bin/env python3
"""Round-7 (invocation 6) close-out: compare the round-6 sweep outputs with round 5
byte-for-byte (ignoring `###` header lines and lake's `[k/N]` job counters) and
emit audit360/round7_summary.json.
"""
import json
import os
import re
...
LOGS = os.path.join(HERE, "logs-round7")
PREV = os.path.join(HERE, "logs-round6")
KINDPREV = os.path.join(HERE, "logs-round6")
CARDS = [
...
def main():
    summary = {"round": 7, "invocation": 6,
               "generated_by": "audit360/round7_summary.py",
               "logs": "audit360/logs-round7", "compared_to": "audit360/logs-round6"}
    cards = {}
    for card in CARDS:
        entry = {}
        for suffix in ("build", "probe", "fullaudit", "kind"):
            ...
            entry[f"{suffix}_identical_to_round6"] = (
    ...
    summary["all_core_identical_to_round6"] = all(
        c["build_identical_to_round6"] and c["probe_identical_to_round6"]
        and c["fullaudit_identical_to_round6"] and c["kind_identical_to_round6"]
    ...
    with open(os.path.join(HERE, "round7_summary.json"), "w") as f:
