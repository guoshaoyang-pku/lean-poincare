#!/usr/bin/env python3
"""Round-8 (invocation 7) close-out: compare the round-8 sweep outputs with round 7
byte-for-byte (ignoring `###` header lines and lake's `[k/N]` job counters) and
emit audit360/round8_summary.json.
"""
import json
import os
import re
...
LOGS = os.path.join(HERE, "logs-round8")
PREV = os.path.join(HERE, "logs-round7")
KINDPREV = os.path.join(HERE, "logs-round7")
CARDS = [
...
def main():
    summary = {"round": 8, "invocation": 7,
               "generated_by": "audit360/round8_summary.py",
               "logs": "audit360/logs-round8", "compared_to": "audit360/logs-round7"}
    cards = {}
    for card in CARDS:
        entry = {}
        for suffix in ("build", "probe", "fullaudit", "kind"):
            ...
            entry[f"{suffix}_identical_to_round7"] = (
    ...
    summary["all_core_identical_to_round7"] = all(
        c["build_identical_to_round7"] and c["probe_identical_to_round7"]
        and c["fullaudit_identical_to_round7"] and c["kind_identical_to_round7"]
    ...
    with open(os.path.join(HERE, "round8_summary.json"), "w") as f:
