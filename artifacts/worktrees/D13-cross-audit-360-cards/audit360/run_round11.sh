#!/bin/bash
# D13-cross-audit-360-cards round-11 (invocation 8) re-verification sweep.
# Rebuilds each available card from the staged source copy, re-runs the generated
# per-declaration axiom probe, the complete-namespace metaprogram audit, the
# declaration-kind screen, the negative control, the semantic-ledger snapshot and
# the A3 D2/D3 counterexample probe.  Logs to audit360/logs-round11/.
set -u
BASE=/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D13-cross-audit-360-cards
LOG="$BASE/audit360/logs-round11"
...
for CARD in $CARDS; do
  ...
  {
    echo "### round11 card=$CARD cwd=$PKG at $(date -Is)"
  ...
  { echo "### round11 probe $CARD at $(date -Is)"; } > "$LOG/$CARD.probe.log" 2>&1
  ...
  if [ -d A3ExtraR3 ]; then
    for f in A3ExtraR3/*.lean; do
      [ -e "$f" ] || continue
      b=$(basename "$f" .lean)
      timeout 3600 lake env lean "$f" > "$LOG/$CARD.extraR3.$b.log" 2>&1
      echo "$?" > "$LOG/$CARD.extraR3.$b.rc"
    done
  fi
  if [ -d A3ExtraR6 ]; then
    for f in A3ExtraR6/*.lean; do
      [ -e "$f" ] || continue
      b=$(basename "$f" .lean)
      timeout 3600 lake env lean "$f" > "$LOG/$CARD.extraR6.$b.log" 2>&1
      echo "$?" > "$LOG/$CARD.extraR6.$b.rc"
    done
  fi
  for XD in A3ExtraR7 A3ExtraR8; do
    if [ -d "$XD" ]; then
      for f in "$XD"/*.lean; do
        [ -e "$f" ] || continue
        b=$(basename "$f" .lean)
  ...
  if [ -d A3ExtraR5 ]; then
    for f in A3ExtraR5/*.lean; do
      [ -e "$f" ] || continue
      b=$(basename "$f" .lean)
      timeout 3600 lake env lean "$f" > "$LOG/$CARD.extraR5.$b.log" 2>&1
      echo "$?" > "$LOG/$CARD.extraR5.$b.rc"
    done
  fi
