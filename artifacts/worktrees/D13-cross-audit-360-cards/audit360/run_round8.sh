...
# the A3 D2/D3 counterexample probe.  Logs to audit360/logs-round8/.
set -u
BASE=/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D13-cross-audit-360-cards
LOG="$BASE/audit360/logs-round8"
...
for CARD in $CARDS; do
  ...
  {
    echo "### round8 card=$CARD cwd=$PKG at $(date -Is)"
  ...
  { echo "### round8 probe $CARD at $(date -Is)"; } > "$LOG/$CARD.probe.log" 2>&1
