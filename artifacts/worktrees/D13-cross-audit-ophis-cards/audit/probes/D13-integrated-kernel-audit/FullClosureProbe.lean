...
run_cmd do
  ...
  let inPackage (m : Name) : Bool :=
    packageRoots.any (fun r => r.isPrefixOf m) && !excluded.contains m
  ...
  IO.println s!"D13XFULL\tmodules\t{sortedMods.size}"
  IO.println s!"D13XFULL\tdeclarations\t{allRows.size}"
  IO.println s!"D13XFULL\ttheorems\t{nThm}"
  IO.println s!"D13XFULL\tpartial_defs\t{nPartial}"
  IO.println s!"D13XFULL\tproject_axioms\t{nAxiom}"
  IO.println s!"D13XFULL\tunsafe\t{nUnsafe}"
  IO.println s!"D13XFULL\tsorry_cones\t{nSorry}"
  IO.println s!"D13XFULL\tnative_cones\t{nNative}"
  IO.println s!"D13XFULL\tunapproved_cones\t{nOther}"
  IO.println s!"D13XFULL\tcollect_failures\t{nFail}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail > 0 then
    IO.println "D13XFULL\tVERDICT\tFAIL"
    throwError "D13XFullClosure: forbidden dependency found"
  else
    IO.println "D13XFULL\tVERDICT\tPASS"
