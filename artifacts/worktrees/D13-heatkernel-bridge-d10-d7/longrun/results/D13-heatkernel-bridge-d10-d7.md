...
> **Thirteenth invocation (2026-09-12T00:10+08:00, ≈ 0.6 h):** the D12 corrected
...
> jobs), per-file 29/29, worktree 350 files 0 failures, forbidden 0 hard / 0 soft, negative
...
| --- | --- | --- |
| source hashes | **28 recorded** (16 `D13` files, 9 `D7` consumers, 3 build-config entries) | `logs/d13_eleventh_final_hashes.txt` |
| semantic `#check`/`#print axioms`/`example` transcripts | **12 files, exit 0, 0 failures** | `logs/d13_eleventh_final_semantic_checks.out` |
| full `lake build` (cwd `release/`) | **exit 0 — `Build completed successfully (9204 jobs)`, D6AUDIT PASS, `D13HeatKernelBridgeAxiomCheck: PASS — all 455 declarations … [propext, Classical.choice, Quot.sound]`** | `logs/d13_eleventh_final_build.log` |
| per-file dispatcher gate (authored files) | **25/25 exit 0, 0 failures** | `logs/d13_eleventh_final_perfile.txt` |
| full worktree gate | **343/343 exit 0, 0 failures** | `logs/d13_eleventh_final_gate_raw.txt` |
...
TASK_DONE — `/data3/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D13-heatkernel-bridge-d10-d7/longrun/results/D13-heatkernel-bridge-d10-d7.md`
(eleventh-invocation gates clean: baseline 26/26 hashes byte-identical, final artifact build **exit 0 (9204 jobs)**,
`D13HeatKernelBridgeAxiomCheck PASS 455/455`, semantic transcripts **12 files exit 0**, per-file
**25/25**, worktree **343/343**, forbidden **0 hard / 0 soft**, negative control PASS, source integrity clean
(**10 expected new-file lines**), upstream snapshot PASS, **28 hashes** recorded and re-verified. The
...
## 24. Twelfth invocation (2026-09-11T23:10+08:00, elapsed ≈ 1.0 h): the Dirichlet gap and the long-time asymptotics of the pinned finite heat kernel
...
**All gates green on the frozen twelfth-invocation artifact** (`logs/d13_twelfth_final_*`):

- **Source hashes**: 30 recorded in `logs/d13_twelfth_final_hashes.txt` — 17
...
- **Semantic transcripts**: 13 files exit 0, 0 failures
...
- **Full build**: exit 0, `Build completed successfully (9206 jobs)`, D6AUDIT
  PASS, `D13HeatKernelBridgeAxiomCheck PASS 511/511` (`logs/d13_twelfth_final_build.log`).
- **Per-file gate**: 26/26 exit 0, 0 failures
  (`logs/d13_twelfth_final_perfile.txt`).
- **Worktree gate**: 346 files, 0 failures (`logs/d13_twelfth_final_gate_raw2.txt`; the first run of this gate
  flagged only the two temporary API-probe files `tmp/d13_twelfth_api_probe2.lean` and `tmp/d13_twelfth_api_probe3.lean`, which
...
- **Source integrity**: `diff -rq release/Poincare` vs `D12-heat-domain-repair` = 11 lines, all
  `Only in` entries — the new `D13` tree (which contains the extended `All.lean` and
  `AxiomAudit.lean`) and the 10 authored D7 consumer files (including the new `ErgodicityStatus.lean`);
  no file present in the D12 tree was modified (`logs/d13_twelfth_final_diff.txt`).
...
## 25. Thirteenth invocation (2026-09-12T00:10+08:00, elapsed ≈ 0.6 h): the weak (test-paired) heat equation on the corrected admissible-test-function domain
...
- **Source hashes**: 33 recorded and re-verified with `sha256sum -c`
  (`logs/d13_thirteenth_final_hashes_check.txt`, all OK);
- **Semantic transcripts**: 14 files exit 0, 0 failures
  (`logs/d13_thirteenth_final_semantic_all.out`), including the extended
  `tmp/d13_semantic_checks_n_weak.lean`;
- **Full build**: exit 0, `Build completed successfully (9209 jobs)`, D6AUDIT PASS,
  `D13HeatKernelBridgeAxiomCheck PASS 556/556` (`logs/d13_thirteenth_final_build.log`);
- **Per-file gate**: 29/29 exit 0, 0 failures (`logs/d13_thirteenth_final_perfile.txt`);
- **Worktree gate**: 350 files, 0 failures (`logs/d13_thirteenth_final_gate_raw.txt`);
