#!/usr/bin/env bash
# D13-heatkernel-bridge-d10-d7: sixth-invocation gate suite.
# Usage: bash tmp/d13_sixth_gates.sh <prefix>   e.g. d13_sixth_baseline / d13_sixth_final
set -u
PREFIX="${1:?usage: d13_sixth_gates.sh <prefix>}"
...
for f in release/Poincare/D13/HeatKernelBridge/Basic.lean \
  ...
  release/Poincare/D13/HeatKernelBridge/LaplacianSymmetryRefutation.lean \
         ...
         release/Poincare/D7/HeatKernel/V1Interface.lean \
         release/Poincare/D7/HeatKernel/StatementStatus.lean \
         release/Poincare/D7/HeatKernel/RepairStatus.lean; do
  if [ -f "$f" ]; then
    timeout 1800 lake env lean "$f" > /dev/null 2>&1
    echo "$?:$f" >> ${L}_perfile.txt
  fi
done
echo "perfile_failures=$(grep -cv '^0:' ${L}_perfile.txt)" >> "$SUM"
