#!/usr/bin/env python3
"""
L4-child-ricci-to-doubling — INDEPENDENT acceptance check (round-2 re-verification).
...
bytes and adds semantic adversarial scans:
  ...
  B. axiom cones: parse the *freshly regenerated* `logs/independent-acceptance-rebuild.log`
...
Exit code 0 iff every gate passes.  Writes `evidence/independent-acceptance.json`.
"""
import hashlib
import json
import math
...
REBUILD_LOG = "logs/independent-acceptance-rebuild.log"
...
def main():
    res = {"schema": "l4-child-ricci-to-doubling/independent-acceptance-v1",
    ...
    out = os.path.join(WT, "evidence/independent-acceptance.json")
