...
**Vendored, unchanged, by task `D7-perelman-conditional-monotonicity`.** The D7 conjugate-heat
certificate layer is an accepted input of this task but is not part of the
`D7-reduced-length-volume` scaffold. Since this worktree may only add files under
`Poincare/D7/Monotonicity/`, the interface declarations are vendored here verbatim so that the
monotonicity assembly can consume the *actual* kernel-checked certificate types and lemmas.
Provenance: `D7-conjugate-heat-interface`, file `Poincare/D7/ConjugateHeat/Basic.lean`.
The original imports the finite weighted-graph module `Poincare.D7.ConjugateHeat.Laplacian`;
that module is *not* used by the declarations below, and its dependency chain (the graph
`Divergence` layer) is not part of this scaffold, so the import is dropped. A self-contained
non-vacuity instance replaces the graph instance in `Nonvacuity.lean`.
-/
