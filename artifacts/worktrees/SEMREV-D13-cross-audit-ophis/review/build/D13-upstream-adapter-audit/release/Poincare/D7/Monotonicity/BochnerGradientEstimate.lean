...
**Vendored, unchanged, by task `D7-perelman-conditional-monotonicity`.** The D7 Bochner
...
`Poincare/D7/Monotonicity/`, the certificate declarations are vendored here verbatim so that
the monotonicity assembly can consume the *actual* kernel-checked certificate types and
lemmas. Provenance: `D7-bochner-formula`, file `Poincare/D7/Bochner/GradientEstimate.lean`.
Only the import path of the vendored companion file is changed.
-/
