...
theorem finiteConjKernelWith_mass_tendsto (G : FiniteHeatOperator X) (R : X → ℝ) (t₀ : ℝ)
  ...
  have hval : (∑ x, (if x = y then (1 : ℝ) else 0)) = 1 := by
...
together with the terminal normalization `∫_M K x y t dV → 1` as `t → t₀⁻`. At `R = 0` the mass law
degenerates to `∂_t ∫ K dV = 0`, so unit mass is the special case; the direction needed here —
every v2 inhabitant is a v3 inhabitant — is proved below
(`isConjugateHeatKernelPDEMassLaw_of_isConjugateHeatKernelPDE`). The PDE field, strict positivity
and the terminal Dirac condition are unchanged from v2, so v3 only admits the additional
solutions with genuinely nonvanishing curvature mass, which v2 excludes by its normalization
field. -/
