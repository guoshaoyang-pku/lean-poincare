/-
SEMREV independent use probe for D13-critical-path-review (written for the SEMREV review).
...
namespace SEMREVUse_D13_critical_path_review
...
def pairs : List (Name × Name) := [
  ("Poincare.D13.CriticalPathReview.b1_dimension_one".toName, "Poincare.D13.CriticalPathReview.kernelTangent_scalarField_iff".toName),
  ("Poincare.D13.CriticalPathReview.b1_dimension_one".toName, "Poincare.D13.CriticalPathReview.scalar_forward_invariance".toName)]

def reverseNames : List Name := [
  "Poincare.D13.CriticalPathReview.b1_dimension_one".toName,
  "Poincare.D12.SurgeryRecognition.iteratedSphereSum_homeo_sphere".toName]

def scanPrefixes : List Name := [
  "Poincare".toName]

end SEMREVUse_D13_critical_path_review

open SEMREVUse_D13_critical_path_review in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"USE_DONE\tD13-critical-path-review"
