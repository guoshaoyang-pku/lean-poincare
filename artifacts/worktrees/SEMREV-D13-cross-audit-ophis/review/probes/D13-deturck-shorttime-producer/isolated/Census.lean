/-
SEMREV independent fail-closed census probe for D13-deturck-shorttime-producer-isolated.
...
namespace SEMREV_D13_deturck_shorttime_producer_isolated
...
end SEMREV_D13_deturck_shorttime_producer_isolated

open SEMREV_D13_deturck_shorttime_producer_isolated in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tdeclarations\t{rows.size}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\ttheorems\t{nThm}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tpartial_defs\t{nPartial}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\topaque\t{nOpaque}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tproject_axioms\t{nAxiom}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tunsafe\t{nUnsafe}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tsorry_cones\t{nSorry}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tnative_decide_cones\t{nNative}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tunapproved_axiom_cones\t{nOther}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tcollect_failures\t{nFail}"
  IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tproof_wanted\t{nProofWanted}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail + nProofWanted > 0 then
    IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tVERDICT\tFAIL"
    throwError "SEMREV: forbidden dependency found (D13-deturck-shorttime-producer-isolated)"
  else
    IO.println s!"XAUDIT\tD13-deturck-shorttime-producer-isolated\tVERDICT\tPASS"
