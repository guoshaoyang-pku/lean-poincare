/-
SEMREV independent fail-closed census probe for D13-heatkernel-bridge-d10-d7-isolated.
...
namespace SEMREV_D13_heatkernel_bridge_d10_d7_isolated
...
end SEMREV_D13_heatkernel_bridge_d10_d7_isolated

open SEMREV_D13_heatkernel_bridge_d10_d7_isolated in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tdeclarations\t{rows.size}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\ttheorems\t{nThm}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tpartial_defs\t{nPartial}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\topaque\t{nOpaque}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tproject_axioms\t{nAxiom}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tunsafe\t{nUnsafe}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tsorry_cones\t{nSorry}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tnative_decide_cones\t{nNative}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tunapproved_axiom_cones\t{nOther}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tcollect_failures\t{nFail}"
  IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tproof_wanted\t{nProofWanted}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail + nProofWanted > 0 then
    IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tVERDICT\tFAIL"
    throwError "SEMREV: forbidden dependency found (D13-heatkernel-bridge-d10-d7-isolated)"
  else
    IO.println s!"XAUDIT\tD13-heatkernel-bridge-d10-d7-isolated\tVERDICT\tPASS"
