/-
SEMREV independent fail-closed census probe for D13-integrated-kernel-audit-isolated.
...
namespace SEMREV_D13_integrated_kernel_audit_isolated
...
end SEMREV_D13_integrated_kernel_audit_isolated

open SEMREV_D13_integrated_kernel_audit_isolated in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tdeclarations\t{rows.size}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\ttheorems\t{nThm}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tpartial_defs\t{nPartial}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\topaque\t{nOpaque}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tproject_axioms\t{nAxiom}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tunsafe\t{nUnsafe}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tsorry_cones\t{nSorry}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tnative_decide_cones\t{nNative}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tunapproved_axiom_cones\t{nOther}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tcollect_failures\t{nFail}"
  IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tproof_wanted\t{nProofWanted}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail + nProofWanted > 0 then
    IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tVERDICT\tFAIL"
    throwError "SEMREV: forbidden dependency found (D13-integrated-kernel-audit-isolated)"
  else
    IO.println s!"XAUDIT\tD13-integrated-kernel-audit-isolated\tVERDICT\tPASS"
