/-
SEMREV independent fail-closed census probe for D13-morgan-tian-adapter-plan-isolated.
...
namespace SEMREV_D13_morgan_tian_adapter_plan_isolated
...
end SEMREV_D13_morgan_tian_adapter_plan_isolated

open SEMREV_D13_morgan_tian_adapter_plan_isolated in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tdeclarations\t{rows.size}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\ttheorems\t{nThm}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tpartial_defs\t{nPartial}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\topaque\t{nOpaque}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tproject_axioms\t{nAxiom}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tunsafe\t{nUnsafe}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tsorry_cones\t{nSorry}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tnative_decide_cones\t{nNative}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tunapproved_axiom_cones\t{nOther}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tcollect_failures\t{nFail}"
  IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tproof_wanted\t{nProofWanted}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail + nProofWanted > 0 then
    IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tVERDICT\tFAIL"
    throwError "SEMREV: forbidden dependency found (D13-morgan-tian-adapter-plan-isolated)"
  else
    IO.println s!"XAUDIT\tD13-morgan-tian-adapter-plan-isolated\tVERDICT\tPASS"
