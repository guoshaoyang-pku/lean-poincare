/-
SEMREV independent fail-closed census probe for D13-upstream-adapter-audit.
...
namespace SEMREV_D13_upstream_adapter_audit
...
end SEMREV_D13_upstream_adapter_audit

open SEMREV_D13_upstream_adapter_audit in
run_cmd do
  let env ← getEnv
  ...
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tdeclarations\t{rows.size}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\ttheorems\t{nThm}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tpartial_defs\t{nPartial}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\topaque\t{nOpaque}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tproject_axioms\t{nAxiom}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tunsafe\t{nUnsafe}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tsorry_cones\t{nSorry}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tnative_decide_cones\t{nNative}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tunapproved_axiom_cones\t{nOther}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tcollect_failures\t{nFail}"
  IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tproof_wanted\t{nProofWanted}"
  if nAxiom + nUnsafe + nSorry + nNative + nOther + nFail + nProofWanted > 0 then
    IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tVERDICT\tFAIL"
    throwError "SEMREV: forbidden dependency found (D13-upstream-adapter-audit)"
  else
    IO.println s!"XAUDIT\tD13-upstream-adapter-audit\tVERDICT\tPASS"
