...
Worktree: /Users/guoshaoyang/Desktop/workdir/math/lean_poincare/longrun/worktrees/D12-heat-domain-repair
...
Objective:
Repair the D7 initial-condition domain, not the heat equation. Read D11 HeatKernelBridge: all continuous functions on noncompact Euclidean space is overstrong. Define a versioned admissible-test-function interface (continuous compact support or an explicitly suitable integrable class), prove the D11 Euclidean core inhabits it for every dimension, and prove precisely scoped compatibility for compact finite-measure settings. Do not assume FullInitialCondition in positive dimension; do not replace genuine Laplacians by arbitrary operators. Keep legacy definitions unchanged until downstream compatibility is proved. If feasible formalize an explicit fast-growing counterexample to the old condition; label the informal obstruction separately until then.

Work only in this isolated worktree. The existing release/ directory is the Lean package. Build from release/, not the parent. Read D7/D10/D11 sources, available result cards, the pinned mathlib source and LONG_PLAN.json. The snapshot may include preliminary dependencies: rebuild before relying on them. Do NOT re-scaffold, delete imported modules, modify tests, change the toolchain, edit global queues/settings, overwrite another worker, or commit/push. Add authored Lean files under release/Poincare/D12/HeatDomain/ and a task-local audit module. If an upstream definition is wrong, create a versioned corrected definition plus proved compatibility in your directory instead of silently changing the target. Published claims remain historical unless your fresh checks cover them.
