...
Worktree: /Users/guoshaoyang/Desktop/workdir/math/lean_poincare/longrun/worktrees/D12-kappa-variational
...
Objective:
Develop a useful sublemma for reduced length, reduced-volume or entropy-to-noncollapsing. Start by checking D7 Reduced/Kappa and D11 ReducedVolume sources. Prove an actual curve-energy bound/minimizer or Gaussian reduced-volume normalization using a real curve/function space; not an arbitrary Prop certificate. If already available, prove one quantitative estimate required for the general argument and identify its geometric assumptions. Separate model-space calculation, conditional transfer and general theorem. Do not infer canonical neighborhoods or ancient-solution classification from a formal record.

Work only in this isolated worktree. The existing release/ directory is the Lean package. Build from release/, not the parent. Read D7/D10/D11 sources, available result cards, the pinned mathlib source and LONG_PLAN.json. The snapshot may include preliminary dependencies: rebuild before relying on them. Do NOT re-scaffold, delete imported modules, modify tests, change the toolchain, edit global queues/settings, overwrite another worker, or commit/push. Add authored Lean files under release/Poincare/D12/KappaVariational/ and a task-local audit module. If an upstream definition is wrong, create a versioned corrected definition plus proved compatibility in your directory instead of silently changing the target. Published claims remain historical unless your fresh checks cover them.
