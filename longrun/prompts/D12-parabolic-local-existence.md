...
Objective:
Work toward genuine short-time PDE existence. Inspect mathlib ODE, Banach fixed point, function-space and D9 DeTurck/ParabolicEstimates APIs. Construct a nontrivial heat-semigroup mild solution or contraction argument on a precisely defined Banach space with verified bounds, rather than finite-dimensional ODE relabeling. Prove the exact bridge obligations for a semilinear parabolic model. Identify the derivative loss and quasilinear regularity barriers to Ricci-DeTurck, never assume the target existence theorem as a field. Research existing open-source formalizations with exact license/revision before reuse; no wholesale import of admitted proofs.

Work only in this isolated worktree. The existing release/ directory is the Lean package. Build from release/, not the parent. Read D7/D10/D11 sources, available result cards, the pinned mathlib source and LONG_PLAN.json. The snapshot may include preliminary dependencies: rebuild before relying on them. Do NOT re-scaffold, delete imported modules, modify tests, change the toolchain, edit global queues/settings, overwrite another worker, or commit/push. Add authored Lean files under release/Poincare/D12/ParabolicLocal/ and a task-local audit module. If an upstream definition is wrong, create a versioned corrected definition plus proved compatibility in your directory instead of silently changing the target. Published claims remain historical unless your fresh checks cover them.
