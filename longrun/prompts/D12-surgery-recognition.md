Task id: D12-surgery-recognition
Worktree: /Users/guoshaoyang/Desktop/workdir/math/lean_poincare/longrun/worktrees/D12-surgery-recognition
...
Objective:
Expand RecognitionHypotheses, ExtinctionCertificate, ConnectedSumDecomposition and SphericalPieceRecognition. Prove one substantive topology/geometry lemma that replaces an assumed field (e.g. a correct covering-space recognition or connected-sum simple-connectivity implication) with an actual construction. Existing 'sphere_of_spheres' and 'recognize' fields are assumptions, not proofs. If unavailable, formalize the smallest nontrivial prerequisite and a reviewed decomposition of neck estimates, surgery existence, finite extinction and final topology. Do not conflate an abstract well-founded decreasing complexity with extinction of an actual Ricci flow.

Work only in this isolated worktree. The existing release/ directory is the Lean package. Build from release/, not the parent. Read D7/D10/D11 sources, available result cards, the pinned mathlib source and LONG_PLAN.json. The snapshot may include preliminary dependencies: rebuild before relying on them. Do NOT re-scaffold, delete imported modules, modify tests, change the toolchain, edit global queues/settings, overwrite another worker, or commit/push. Add authored Lean files under release/Poincare/D12/SurgeryRecognition/ and a task-local audit module. If an upstream definition is wrong, create a versioned corrected definition plus proved compatibility in your directory instead of silently changing the target. Published claims remain historical unless your fresh checks cover them.
