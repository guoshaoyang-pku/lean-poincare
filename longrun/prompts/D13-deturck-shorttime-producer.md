...
Worktree: /data/home/guoshaoyang/workdir/lean_poincare/longrun/worktrees/D13-deturck-shorttime-producer

Model: deepseek-flash; minimal preset, max effort; never switch model silently.
Read the pinned Frenzymath snapshot under `third_party/frenzymath/Poincare-Conjecture` (commit bb91a091) and `docs/UPSTREAM-INTEGRATION.md`; reuse upstream packages instead of recreating foundations. Continue from checkpoint.json; update checkpoint.json at least every 30 minutes and keep longrun/results/D13-deturck-shorttime-producer.md current.
...
Deliver longrun/results/D13-deturck-shorttime-producer.md and .json with proved_declarations, expanded_hypotheses, semantic_class, exact_blockers_closed, remaining_blockers, source_hashes, compile_evidence, axiom_evidence, next_dependency_requests, elapsed time. End TASK_DONE only if the milestone is fully checked; otherwise TASK_BLOCKED or checkpoint and TASK_IN_PROGRESS.

Objective: Produce the analytic antecedent for Hamilton/DeTurck short-time existence: a RicciDeTurckPicardModel producer from an arbitrary smooth metric, building on D12-parabolic-local-existence (semigroup + Banach fixed point), D9 strict-parabolicity certificate, and D13-topping-ricci-adapter-plan split architecture. This is the missing half of U8.
