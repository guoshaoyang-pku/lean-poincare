...
* `ricciForm B x y` — do Carmo's `Q(x,y)`, defined basis-free as the trace of the
  ...
  (from the pair-swap symmetry of `B`).  Being a trace, `Q` is basis-independent,
  which is do Carmo's argument that his normalized `Ric_p(x) = Q(x,x)/(n-1)` is
  well defined.

**Normalization.** This file — and DoCarmoLib generally — is **unnormalized**:
`ricciForm` is the raw trace `∑_i B x e_i y e_i` and `scalarCurvature` the raw
trace of that.  Neither the `1/(n-1)` of do Carmo's `Ric_p` nor the `1/n(n-1)` of
his `K(p)` is ever divided out, and there is **no `Ric_p` declaration**: the
displays above name do Carmo's quantities to fix the correspondence, not Lean
definitions.  A consumer wanting do Carmo's normalized bound must divide
explicitly; `DoCarmoCh4RicciSectional.lean` states the sectional-to-Ricci
inequality in the unnormalized `(n-1)·k` form for exactly that reason.
