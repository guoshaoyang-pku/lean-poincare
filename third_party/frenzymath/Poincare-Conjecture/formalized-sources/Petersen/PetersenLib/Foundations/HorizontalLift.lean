...
/-!
# Horizontal lifts of surjective linear maps

Let `B` be a positive-definite bilinear form on a finite-dimensional real
vector space `E` and let `A : E →L[ℝ] E'` be surjective.  The **horizontal
lift** `horizontalLift B A : E' →L[ℝ] E` is the unique right inverse of `A`
whose range is `B`-orthogonal to `ker A`; equivalently, it is the inverse of
the isomorphism `A|_{(ker A)^⊥} : (ker A)^⊥ ≅ E'`.  This is the linear-algebra
heart of the theory of Riemannian submersions (Petersen §1.3): the metric on
the base of a Riemannian submersion is defined by transporting the metric of
the total space through the horizontal lift of the differential.

The point of this file is not existence — which is elementary and already
available as `PetersenLib.exists_preimage_orthogonal_ker` — but **smoothness in
the pair `(B, A)`** (`contDiffAt_horizontalLift`).  That is what a *quotient
metric* needs and what a bare existence statement cannot give: to know that the
quotient metric is a smooth section one must know that the horizontal
distribution `p ↦ (ker Dq_p)^⊥` varies smoothly with `p`.

The construction is coordinate-free, and in particular chooses no inner
product on the target `E'`.  The trick is that a bilinear form
`B : E →L[ℝ] E →L[ℝ] ℝ` *is literally its own Riesz map* `E → E*`, invertible
exactly when `B` is nondegenerate.  Writing `Aᵗ : (E')* →L[ℝ] E*` for the
transpose, put
...
`ContinuousLinearMap.inverse` is smooth at invertible points
(`ContinuousLinearMap.IsInvertible.contDiffAt_map_inverse`), so the whole
formula is smooth in `(B, A)`.  In matrix notation this is the familiar
`L = B⁻¹Aᵗ(AB⁻¹Aᵗ)⁻¹`, the `B`-weighted Moore–Penrose right inverse.

## Main declarations

* `PetersenLib.horizontalLift` — the horizontal lift `E' →L[ℝ] E`.
* `PetersenLib.horizontalLift_rightInverse` — `A (horizontalLift B A u) = u`.
* `PetersenLib.horizontalLift_horizontal` — `B (horizontalLift B A u) w = 0` for `w ∈ ker A`.
* `PetersenLib.horizontalLift_unique` — the two properties above characterize it.
* `PetersenLib.contDiffAt_horizontalLift` — smooth dependence on `(B, A)`.
-/
