...
/-!
# Topping Chapter 5: the non-autonomous gauge flow
...
/-- **Math.** At every time in its flow-box interval, the spatial flow is
continuous in the initial point. -/
theorem TimeDependentFlowBox.continuous_spatialFlow
    {V : SmoothTimeDependentVectorField (I := I) (M := M)} {t₀ : ℝ}
    (B : TimeDependentFlowBox (I := I) (M := M) V t₀)
    {s : ℝ} (hs : s ∈ Ioo (-B.eta) B.eta) :
    Continuous (fun p : M => B.spatialFlow p s) := by
  have h := B.continuousOn_spatialFlow.comp_continuous
    (continuous_id.prodMk continuous_const)
    (fun p : M => ⟨mem_univ p, hs⟩)
  change Continuous (fun p : M => B.spatialFlow p s)
  exact h
...
/-- **Math.** On overlapping compact-slice flow boxes, the spatial slice map
is a global homeomorphism.  Its inverse is the reverse-time spatial flow from
the box based at `t₁`. -/
theorem TimeDependentFlowBox.spatialFlow_homeomorph_of_common
    {V : SmoothTimeDependentVectorField (I := I) (M := M)}
    {t₀ t₁ s : ℝ}
    (B₀ : TimeDependentFlowBox (I := I) (M := M) V t₀)
    (B₁ : TimeDependentFlowBox (I := I) (M := M) V t₁)
    ...
    (hs₁ : -s ∈ Ioo (-B₁.eta) B₁.eta) :
    ∃ e : Homeomorph M M, ∀ p : M, e p = B₀.spatialFlow p s := by
  have hleft : ∀ p : M,
      B₁.spatialFlow (B₀.spatialFlow p s) (-s) = p := by
    intro p
  ...
  have hright : ∀ p : M,
    ...
    simpa using h
  have hcont₀ : Continuous (fun p : M => B₀.spatialFlow p s) := by
    have h := B₀.continuousOn_spatialFlow.comp_continuous
      (continuous_id.prodMk continuous_const)
      (fun p : M => ⟨mem_univ p, hs₀⟩)
    change Continuous (fun p : M => B₀.spatialFlow p s)
  ...
  exact ⟨e, fun p => by rfl⟩

/-! ## Cocycle law on overlapping time slices -/

/-- **Math.** Two consecutive spatial flow-box steps agree with the direct
flow-box step whenever all three elapsed times are defined.  The equality is
obtained by translating the first suspended trajectory and applying uniqueness
on the intersection of the translated and second flow-box intervals. -/
theorem TimeDependentFlowBox.spatialFlow_comp_of_common
    {V : SmoothTimeDependentVectorField (I := I) (M := M)}
    {t₀ t₁ s u : ℝ}
    (B₀ : TimeDependentFlowBox (I := I) (M := M) V t₀)
    (B₁ : TimeDependentFlowBox (I := I) (M := M) V t₁)
    (ht : t₁ = t₀ + s) (p : M)
    (hs : s ∈ Ioo (-B₀.eta) B₀.eta)
    (hu : u ∈ Ioo (-B₁.eta) B₁.eta)
    (hsu : s + u ∈ Ioo (-B₀.eta) B₀.eta) :
    B₁.spatialFlow (B₀.spatialFlow p s) u =
      B₀.spatialFlow p (s + u) := by
  let a : ℝ := max (-B₁.eta) (-B₀.eta - s)
  let b : ℝ := min B₁.eta (B₀.eta - s)
  have ha0 : a < 0 := by
    dsimp [a]
  ...
  have h0b : 0 < b := by
    ...
    · exact B₁.eta_pos
    · linarith [hs.2]
  have hsu_shift : u + s ∈ Ioo (-B₀.eta) B₀.eta := by
    simpa [add_comm] using hsu
  have huab : u ∈ Ioo a b := by
    constructor
    · dsimp [a]
      rw [max_lt_iff]
      constructor
      · exact hu.1
      · linarith [hsu_shift.1]
    · dsimp [b]
      rw [lt_min_iff]
      constructor
      · exact hu.2
      · linarith [hsu_shift.2]
  have hshift_subset : Ioo a b ⊆
      {r : ℝ | r + s ∈ Ioo (-B₀.eta) B₀.eta} := by
    intro r hr
    change r + s ∈ Ioo (-B₀.eta) B₀.eta
  ...
  have hleft : ((γ₀ ∘ (· + s)) u).1 =
      B₀.spatialFlow p (s + u) := by
    simp [γ₀, TimeDependentFlowBox.spatialFlow, Function.comp_apply, add_comm]
  have hright : (B₁.Φ q u).1 =
      B₁.spatialFlow (B₀.spatialFlow p s) u := by
    rfl
  rw [hleft, hright] at hfst
  exact hfst.symm
